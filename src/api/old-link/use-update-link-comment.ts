...
): UseMutationResult<InfiniteData<PaginatedData<Comment>>, Error, Partial<Document<Comment>>, Document<Comment>> => {
  const queryClient = useQueryClient();
  const commentsKey = queryKeys.linkComments(link.id as string);

  return useMutation((commentToUpdate) => updateLinkComment(link.id, prevComment.id, commentToUpdate), {
    ...
    onError: (err, variables, prevComment) => {
      ...
      if (prevComment) {
        queryClient.setQueryData<InfiniteData<PaginatedData<Comment>>>(commentsKey, (oldComments) =>
          updateItemInsidePaginatedData(prevComment, oldComments)
        );
      }
    },
  });
