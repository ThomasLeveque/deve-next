...
export const useUpdateLink = (
  ...
  queryKey: QueryKey
): UseMutationResult<InfiniteData<PaginatedData<Link>>, Error, Partial<Document<Link>>, Document<Link>> => {
  const queryClient = useQueryClient();
  const updateCategory = useUpdateCategory();

  const [linkToCommentModal, setLinkToCommentModal] = useLinkToCommentModal();

  return useMutation((linkToUpdate: Partial<Document<Link>>) => updateLink(prevLink.id, linkToUpdate), {
    ...
    onSuccess: (data, linkToUpdate, prevLink) => {
      if (linkToUpdate.categories) {
        ...
        prevTags.forEach((prevTag) => {
          const found = newTags.find((newTag) => newTag.toLocaleLowerCase() === prevTag.toLocaleLowerCase());
          if (!found) {
            // Removed item
            const removedItem = tags?.find((tag) => tag.name.toLocaleLowerCase() === prevTag.toLocaleLowerCase());
            if (removedItem) {
        ...
        newTags.forEach((newTag) => {
          const found = prevTags.find((prevTag) => prevTag.toLocaleLowerCase() === newTag.toLocaleLowerCase());
          if (!found) {
            // Added item
            const addedItem = tags?.find((tag) => tag.name.toLocaleLowerCase() === newTag.toLocaleLowerCase());
            if (addedItem) {
              updateCategory.mutate({
